...
<div className="hidden lg:flex items-center justify-center flex-1 gap-6 px-8 h-full">            {navLinks.map((link) => (
    ...
    <div className="flex items-left relative h-full">
      <button
              onClick={toggleTheme}
              className="px-2 py-2 rounded-full  bg-gray-100 dark:bg-white/5 text-slate-600 dark:text-cyan-400 hover:scale-110 transition-all shadow-sm"
            >
              {theme === "dark" ? <Sun size={19} /> : <Moon size={19} />}
            </button>

<div className="hidden lg:flex items-center relative group/lang pl-4 h-full">            <button className="flex items-center gap-1 text-slate-600 dark:text-slate-300 hover:text-cyan-600 dark:hover:text-cyan-400 font-black transition-all">
